...
//!
//! This module defines the canonical metadata schema stored with each vector
//! in LanceDB, enabling rich filtering during vector search.
//!
//! ## Schema (v1)
//!
//! The v1 schema is defined in Phase 8.1 and includes:
//! - `id`: Stable chunk identifier
//! - `base`: Knowledge base name (code, docs, memory)
//! - `branch`: Git/GIK branch name
//! - `source_type`: Source kind (file, memory, url)
//! - `path`: File path or logical key
//! - `tags`: User-defined tags
//! - `revision_id`: Timeline revision reference
//! - `created_at` / `updated_at`: Timestamps
//!
//! Future phases MAY add new fields using LanceDB's schema evolution support.
...
///
/// This struct defines the v1 metadata schema for vector index entries.
/// It is used by both the embedding/chunking pipeline and the LanceDB backend
/// for storage and filtered retrieval.
///
/// ## Field Semantics
///
/// - `id`: Stable identifier for the chunk (content-addressable or sequential).
/// - `base`: Knowledge base name: "code", "docs", "memory", etc.
/// - `branch`: Git/GIK branch name (optional, usually "main").
/// - `source_type`: Kind of source: "file", "memory", "url", "archive".
/// - `path`: For files, the relative path. For memory, a logical key like "memory:{id}".
/// - `tags`: User-defined tags for filtering (empty by default).
/// - `revision_id`: Timeline revision that created this vector.
/// - `created_at`: When the vector was first indexed.
/// - `updated_at`: When the vector was last updated.
...
///
/// ```ignore
/// let filter = VectorSearchFilter::default()
///     .with_base("code")
///     .with_path_prefix("src/");
///
/// let results = index.query_filtered(&embedding, 10, Some(filter))?;
/// ```
...
impl VectorSearchFilter {
    ...
    pub fn to_lance_filter(&self) -> Option<String> {
        ...
        // LanceDB uses arrow array syntax; we need to check if each tag exists
