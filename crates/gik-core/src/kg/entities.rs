...
//! - [`KgNode`] - A node in the knowledge graph representing files, modules, symbols, concepts, etc.
//! - [`KgEdge`] - An edge representing a relationship between two nodes
//! - [`KgStats`] - Aggregate statistics for the knowledge graph
//!
//! ## Storage Format
//!
//! - Nodes are stored in `nodes.jsonl` (one JSON object per line)
//! - Edges are stored in `edges.jsonl` (one JSON object per line)
//! - Stats are stored in `stats.json` (single JSON object)
//!
//! ## JSON Field Names
//!
...
impl KgEdge {
    ...
    fn edge_id_component(from: &str, to: &str) -> String {
        // Use a shortened hash to keep IDs manageable
...
///
/// Stored in `stats.json` within the KG directory.
///
/// ## Fields
///
...
mod tests {
    ...
    #[test]
    fn test_kg_node_deserialization() {
        let json_str = r#"{
            "id": "file:src/lib.rs",
            "kind": "file",
            "label": "src/lib.rs",
        ...
        let node: KgNode = serde_json::from_str(json_str).unwrap();

        assert_eq!(node.id, "file:src/lib.rs");
        assert_eq!(node.kind, "file");
        assert_eq!(node.label, "src/lib.rs");
        assert_eq!(node.props, json!({"lines": 50}));
    ...
    #[test]
    fn test_kg_edge_deserialization() {
        let json_str = r#"{
            "id": "edge:123",
            "from": "fn:main",
            "to": "fn:helper",
        ...
        let edge: KgEdge = serde_json::from_str(json_str).unwrap();

        assert_eq!(edge.id, "edge:123");
        assert_eq!(edge.from, "fn:main");
        assert_eq!(edge.to, "fn:helper");
    ...
    #[test]
    fn test_kg_stats_deserialization() {
        let json_str = r#"{
            "nodeCount": 150,
            "edgeCount": 400,
            "lastUpdated": "2025-11-28T10:00:00Z",
        ...
        let stats: KgStats = serde_json::from_str(json_str).unwrap();

        assert_eq!(stats.node_count, 150);
        assert_eq!(stats.edge_count, 400);
        assert_eq!(stats.version, "kg-v1");
    }
}
